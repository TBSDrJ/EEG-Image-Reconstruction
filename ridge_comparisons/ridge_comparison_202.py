...
class MyLoss(torch.nn.Module):
    ...
    def forward(self, preds: torch.Tensor, targets: torch.Tensor, 
        ...
        wgts = model.get_parameter('linear.weight')
        bias = model.get_parameter('linear.bias')
        loss_1 = wgts.square().sum() + bias.square().sum()
        loss_2 = 0
...
class MyModel(torch.nn.Module):
    ...
    def __init__(self, duration, *args, **kwargs):
        ...
        self.linear = torch.nn.Linear(input_size, 91168)
